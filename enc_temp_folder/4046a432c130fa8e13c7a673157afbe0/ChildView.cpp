...
void CChildView::OnPaint()  // 한번 화면 렌더링 될 떄 이함수 한번 호출됨 알아서 호출됨
{
...
afx_msg void CChildView::OnMyPaint(CDC* dc) { //여기서 그려
	// 현재 시간 표시
...
void CChildView::OnMouseMove(UINT nFlags, CPoint point) {
	...
	if (m_bDrawMode) {
		...
		for (size_t i = 1; i < m_curves.size(); ++i) {
			dc.MoveTo(m_curves[i - 1]);
			dc.LineTo(m_curves[i]);
		}
		//dc.MoveTo(m_ptStart);
		//dc.LineTo(point);
		//m_ptStart = point;
